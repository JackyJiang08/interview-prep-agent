...
Exploratory work only. Nothing here is imported by `src/`, and nothing here is
on the tested path.
...
This directory is empty at present. The pipeline is small enough to explore from
the command line, and an empty notebook would say nothing that
`docs/METHODOLOGY.md` does not already say.
